...
public class StackText {
    public static Text handlePatternText(PlayerEntity player, String message, CallbackInfo ci) {
        ...
        if (Objects.equals(Registries.ITEM.getId(stack.getItem()).toString(), "minecraft:air")) {
            Text msg = Text.translatable("commands.enchant.failed.itemless", player.getDisplayName()).formatted(Formatting.RED);
        ...
        return Text.translatable("chat.type.text", player.getDisplayName(), completeText);
    }

    public static Text getStackText(ItemStack stack) {
        MutableText mutableText = Text.translatable("entity.minecraft.item").append(":").append(stack.getName());
