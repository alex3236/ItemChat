...
public class StackText {
    ...
    public static Text getStackText(ItemStack stack) {
        Text text = (new TranslatableText("entity.minecraft.item").append(":")).append(stack.getName());
        ...
        if (!stack.isEmpty()) {
            ...
            text2.formatted(stack.getRarity().formatting).styled(style -> style.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_ITEM, new LiteralText(compoundTag.toString()))));
        }
        return text2;
    }
}